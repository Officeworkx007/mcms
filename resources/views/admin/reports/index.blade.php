...
@section('content')
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
        @foreach ($reportTypes as $key => $report)
            <button type="button"
                onclick="{{ match ($report['fields']) {
                    'mediator' => 'openMediatorModal',
                    'category' => 'openCategoryModal',
                    default => 'openReportModal',
                } }}('{{ $key }}', @js($report['title']), @js($report['description']))"
    ...
    {{-- "Generate Case-wise Report" modal --}}
    <div id="categoryReportModal" class="hidden fixed inset-0 z-50 items-start justify-center pt-16 px-4">
        <div class="absolute inset-0 bg-black/40" onclick="closeCategoryModal()"></div>

        <div class="relative bg-white w-full max-w-md rounded-lg shadow-xl overflow-hidden">
            <div class="px-5 py-4 border-b border-border flex items-start justify-between">
                <div class="min-w-0 pr-3">
                    <p id="category-modal-report-title" class="text-sm font-bold text-ink">Report Title</p>
                    <p id="category-modal-report-desc" class="mt-1 text-xs text-muted leading-relaxed">Report description
                    </p>
                </div>
            ...
            <form action="{{ route('admin.reports.generate') }}" method="POST" class="px-5 py-4 space-y-3">
                @csrf
                <input type="hidden" id="category-modal-report-key" name="report_key" value="">
                ...
                <p class="text-[11px] text-muted -mt-1">Shows every mediator with cases in this category, regardless of
                    status.</p>
...
@push('scripts')
    <script>
        ...
        function openCategoryModal(key, title, desc) {
            document.getElementById('category-modal-report-key').value = key;
            document.getElementById('category-modal-report-title').textContent = title;
            document.getElementById('category-modal-report-desc').textContent = desc;
